...
type Options = {
	/**
	 * What method to use when evaluating expressions inside template strings?
	 *
	 * By default, ecsstatic will try its best (`best-attempt`) to resolve relative imports. Note that you cannot
	 * currently import class names generated by ecsstatic in other files and use them inside template literals.
	 *
	 * By default, packages are also not resolved (everything is "external"-ized) because it is faster this way.
	 * If you'd like to use an npm package, you can pass it to the `resolvePackages` field inside `evaluateExpressions`.
	 *
	 * Frankly, all this is a pain in the ass and has an infinite number of edge cases, so you might see cryptic
	 * errors, especially if your project is very complex and relies on other vite plugins. This is why you have
	 * the option of keeping things simple by telling ecsstatic to only resolve expressions within the same file
	 * (`same-file`) or even disabling all evaluation of expressions entirely (`false`).
	 *
	 * @default 'best-attempt'
	 */
	evaluateExpressions?:
		| 'best-attempt'
		| 'same-file'
...
export function ecsstatic(options?: Options) {
	const evaluateExpressions = options?.evaluateExpressions ?? 'best-attempt';
	const shouldResolveImports = evaluateExpressions !== false && evaluateExpressions !== 'same-file';
	const esbuildNoExternals =
		typeof evaluateExpressions === 'object' ? evaluateExpressions.resolvePackages ?? [] : [];
	...
	return <Plugin>{
		...
		async transform(code, id) {
			...
			for (const node of cssTemplateDeclarations) {
				...
				if (quasi.expressions.length && shouldResolveImports && !inlinedVars) {
