...
type Options = {
	/**
	 * Should ecsstatic attempt to evaluate expressions (including other class names) defined in the same file?
	 *
	 * This option is pretty safe so it's on by default, but you have the option to turn it off, because why not 🤷
	 *
	 * @default true
	 */
	evaluateSameFileExpressions?: boolean;
	/**
	 ...
	 * To use an npm package, you can pass its name as an array here.
	 *
...
export function ecsstatic(options: Options = {}) {
	const {
		evaluateSameFileExpressions = true,
		resolvePackages = [],
		resolveRelativeImports = evaluateSameFileExpressions && resolvePackages.length,
	} = options;
	...
	return <Plugin>{
		...
		async transform(code, id) {
			...
			for (const node of cssTemplateDeclarations) {
				...
				if (quasi.expressions.length && resolveRelativeImports && !inlinedVars) {
				...
				const templateContents = evaluateSameFileExpressions
